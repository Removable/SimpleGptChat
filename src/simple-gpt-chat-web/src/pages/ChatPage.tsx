...
const ChatPage = () => {
  ...
  const commonSendFunc = async (
    newChatInfo: ChatInfo,
    existChatInfos: ChatInfo[],
  ): Promise<string> => {
    setThinking(true);
  ...
  const callSendMsg = (param: SendMsgArg) =>
    myAxios.post<string>('/api/Chat/SendMessage', param, 'fromBody');
  ...
  const onModeChange = (isChecked: boolean) => {
    setIsTextMode(isChecked);
  };
  ...
  return (
    <div className={'w-full overflow-auto h-[calc(100vh-100px)]'} ref={scrollRef}>
      ...
      {storedChatInfos.map((chatInfo, idx) => {
        return (
          <ChatBox
            ...
            isAudioMode={!isTextMode}
            readMessage={readMessage}
            isReplying={idx === storedChatInfos.length - 1 && replying}
          />
        );
      })}
      {storedChatInfos.length > 0 ? <div className="h-[100px]" /> : <></>}
...
const AudioRecorder = (props: AudioRecorderProps) => {
  ...
  const send = async () => {
    ...
    const storedChatInfos = props.storedChatInfos();

    const tempStoredChatInfos = [...storedChatInfos, newChatInfo];
    props.setStoredChatInfos(tempStoredChatInfos);
    const sendMsgRes = await props.commonSendFunc(newChatInfo, storedChatInfos);
    setLoading(false);
    setAudioBlob(null);
    setRecordedTime(0);

    props.readMessage(sendMsgRes);
  };
...
interface AudioRecorderProps {
  ...
  commonSendFunc: (newChatInfo: ChatInfo, storedChatInfos: ChatInfo[]) => Promise<string>;
